...
bool AppDelegate::applicationDidFinishLaunching()
{
    ...
    if(!glview) {
        ...
        glview = cocos2d::GLViewImpl::createWithRect("zombie-zoo", Rect(0,0,viewSize.width,viewSize.height));
...
#else
    pStack->setXXTEAKeyAndSign("_FF#@LEAVL_M0X", strlen("_FF#@LEAVL_M0X"), "levelmax", strlen("levelmax"));
    pStack->loadChunksFromZIP("res/game.zip");

    string path = FileUtils::getInstance()->fullPathForFilename("res/game.zip");
#endif
...
//    pEngine->executeScriptFile(path.c_str());
    ...
    pEngine->executeString("require(\"app.game\").new():run()");
    return true;
